...
def main():
    ...
    # A handover bisect deliberately hands the guest to the JIT part-way
    # through, so the first transition is the point of the run rather than a
    # failure. Without this the runner aborts at the next two-second poll,
    # which is the Python half of the same reason the last bisect measured
    # nothing: -NoStrict lets the emulator hand over, and this lets the
    # observation survive long enough to record whether it recovered.
    parser.add_argument("--allow-jit", action="store_true",
                        help="do not abort when the static backend hands over to the JIT")
    ...
    try:
        ...
        while time.monotonic() - started < args.seconds:
            ...
            if not args.baseline and not args.allow_jit and result["max_jit_transitions"]:
    ...
    backend_ok = (not result["static_backend_active"]) if args.baseline else (
        result["static_backend_active"]
        and (args.allow_jit or result["max_jit_transitions"] == 0))
