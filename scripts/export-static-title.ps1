...
# Refuse a 32-bit title. The AArch64 recompiler does not reject A32 - every word
# decodes as *something* - so exporting an NX32 image "succeeds" and produces
# modules that compile and then kill the emulator the instant its rtld image is
# entered. That is exactly what happened to Mario Kart 8: the exporter read the
# cartridge's NX32 base build instead of the NX64 build from the installed
# update, and the only visible symptom was a crash on load two days later.
#
# main.npdm's META header carries the answer at offset 0x0C: bit 0 is
# Is64BitInstruction. 0x07 is a 64-bit title, 0x04 a 32-bit one. Checked here as
# well as in the exporter so an older suyu.exe cannot slip a bad export through.
$npdm = Get-ChildItem -LiteralPath $exefs -Recurse -Filter 'main.npdm' -File -ErrorAction SilentlyContinue |
        Select-Object -First 1
if ($npdm) {
    $bytes = [System.IO.File]::ReadAllBytes($npdm.FullName)
    if ($bytes.Length -gt 0x0C) {
        $flags = $bytes[0x0C]
        if (($flags -band 1) -eq 0) {
            throw ("exported a 32-bit (NX32) image: $($npdm.FullName) META flags 0x{0:X2}, " +
                   "Is64BitInstruction=0. The AArch64 recompiler cannot translate A32. If this " +
                   "title has a 64-bit update, the exporter must resolve ExeFS through " +
                   "PatchManager::PatchExeFS so the update is seen." -f $flags)
        }
        Write-Host ("npdm META flags 0x{0:X2} - 64-bit image" -f $flags) -ForegroundColor DarkGray
    }
}
...
function Get-ModuleDigest([string]$Path) {
    ...
    # ToHexString is .NET 5+; Windows PowerShell 5.1 still has to run this.
